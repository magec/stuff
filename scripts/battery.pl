...
# modified by Seynthan Thanapalan (seynthan.tx@gmail.com)
...
# Get full capacity
open(INFO, "< /proc/acpi/battery/BAT0/info") or die "Couldn't open info: ".$!;
foreach (<INFO>) {
...
# Get current state
open(STATE, "< /proc/acpi/battery/BAT0/state") || die $!;
foreach(<STATE>) {
